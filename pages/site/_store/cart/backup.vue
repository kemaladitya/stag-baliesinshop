<template>
  <v-card class="pt-12">
    ...
    <v-card
      v-if="(cart.length || dates.length) && customized_values == null && store.recurring_purchase"
      id="b-subs-mode"
      class="mt-2 pr-0"
    ...
    <v-card
      v-if="!customized_values && !store.recurring_purchase"
      color="white"
      height="10px"
      flat
      tile
...
export default {
  ...
  async mounted () {
    try {
      ...
      // console.log(JSON.stringify(cart_detail, null, 2), ' cart_detail')

      // if ('results' in cart_detail && cart_detail.results.type == 'pkg-order' && cart_detail.results.items.length) {
      //   this.$store.dispatch('setState', {
      //     payload: {
      //       key : 'customized_values',
      //       data: cart_detail.results.items[0].items
      //     }
      //   })
      // }
    ...
    async cart_detail() {
      try {
        ...
        if (cart && cart.status && cart.results) {
          ...
          if (cart.results.type === 'box-order' || cart.results.type === 'parcel-order') {
            const list_skus = []
            let pkg_total   = 0

            cart.results.items.forEach(({ items }) => {
              items.forEach(el => {
