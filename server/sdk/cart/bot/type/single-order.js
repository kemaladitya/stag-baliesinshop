...
async function main({ bot_id, chatkey, method, product_detail_id, qty }) {
  try {
    ...
    const list_items  = await get_cart(`${chatkey}/${information.store.bot_name}`, '')
    const base_value  = { type : 'single-order', items: [], voucher: list_items ? list_items.voucher : null }
    const item        = { ...information.product, qty }

    let action

    switch (method) {
      case 'add'   : action = await add_cart(list_items, item); break;
      case 'reduce': action = await red_cart(list_items, item); break;
      case 'remove': action = await rem_cart(list_items, item); break;

      default: action = await get_cart(`${chatkey}/${information.store.bot_name}`, '');
        break;
    }
    ...
    const set_resp = await set_cart(`${chatkey}/${information.store.bot_name}`, '', base_value)
...
async function add_cart(list_items, item) {
  try {
    ...
    if (!list_items || (list_items.hasOwnProperty('items')) && !list_items.items) {
      return [item]
    }
    ...
    if (is_exist_item.length) {
      for (let i = 0; i < list_items.items.length; i++) {
        ...
        const match_item = item.id === element.id
