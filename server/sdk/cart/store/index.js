...
async function manage(params) {
  try {
    ...
    if (params.method === 'get') {
      manager = await get_cart(`${params.info.store.uuid}/${params.info.store.name}`, '')
    } else {
      switch (params.info.mode) {
        case 'single-order'       : manager = await single_mode(params)       ; break;
        case 'package-order'      : manager = await package_mode(params)      ; break;
        case 'subscription-order' : manager = await subscription_mode(params) ; break;
      }
    }
