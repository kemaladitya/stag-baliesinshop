...
async function main({ method, info }) {
  try {
    const list_items = await get_cart(`${info.store.uuid}/${info.store.name}`, '')
    const base_value = { type : info.mode, items: [], voucher: list_items ? list_items.voucher : null }
    let action

    switch (method) {
      case 'set'   : action = info.item; break;
      case 'add'   : action = await add_cart(list_items, info); break;
      case 'reduce': action = await red_cart(list_items, info); break;
      case 'remove': action = await rem_cart(list_items, info); break;

      default: action = await get_cart(
