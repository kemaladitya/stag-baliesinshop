...
const { get_cart, set_cart, del_cart, check_redis_status } = require("../sdk/handler")

async function create(request, response) {
  try {
    let get_redis    = await get_cart(`${request.body.uuid}/${request.body.bot_name}`, "")
    const check_cart = await check_redis_status(`${request.body.uuid}/${request.body.bot_name}`, get_redis)
    ...
    if (post_order.status !== 200) {
      return response.status(404).json({ error: "create order failed", status: false })
    }

    await set_cart(`${request.body.uuid}/${request.body.bot_name}`, "/last", get_redis)
...
async function reorder_(bot_id, chatkey, outlet, source) {
  try {
    const { info: { bot_name, store_id } } = await axios({
