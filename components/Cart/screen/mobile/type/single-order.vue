  ...
  <div>
    <div v-if="cart.length">
      <div v-for="(item, index) in parsed_lists.product" :key="index">
        <v-card class="d-flex flex-row ma-1 mb-2 pr-1" style="border-radius: 8px;" outlined>
          <v-card
            class="ma-1 pt-9"
            width="25"
            flat
          ...
          <v-card class="d-flex flex-row" flat width="100%">
            <v-card class="ma-2 ml-0" style="border-radius: 8px;" flat outlined>
              <v-img :src="item.main_image" width="85" height="85" loading=lazy style="border-radius: 8px;" />
            </v-card>
              ...
              <div style="min-height: 18px">
                <div v-show="item.express_delivery && is_available_express">
                  <div style="color: #388E3C; font-weight: 600; font-size: 12px; align-self: center">
                    <v-icon color="#388E3C" small>mdi-clock-fast</v-icon>
                    EXPRESS DELIVERY
                  </div>
                </div>
              </div>
